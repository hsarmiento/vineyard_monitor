...
class Wind_gauge extends CI_Controller {
	...
	public function show($vineyard_name,$pcb_id, $day)
	{
		...
		$aTrending = $this->wind_gauge_model->get_wind_trending($pcb_id, $day);
		...
		foreach ($aTrending as $trending) {
				$aData['aWind'][] = "[".(mktime(date("H", strtotime($trending['created_at']))-6, date("i", strtotime($trending['created_at'])), date("s", strtotime($trending['created_at'])), date("m", strtotime($trending['created_at'])), date("d", strtotime($trending['created_at'])), date("Y", strtotime($trending['created_at'])))*1000).",".$trending['temp_value']."]";
